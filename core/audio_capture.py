...
class AudioCapture:
    ...
    def start(self) -> None:
        ...
        logger.info(
            "[AUDIO] Starting audio capture  (device=%s, rate=%d, chunk=%ds)",
        ...
        try:
            ...
            logger.error(
                "[AUDIO] Failed to open audio input stream: %s", exc
            )
            raise
    ...
    def _audio_callback(
        ...
        try:
            if status:
                logger.warning("[AUDIO] Audio callback status: %s", status)
            ...
            # sounddevice callbacks must not throw
            logger.error("[AUDIO] Error in audio callback: %s", exc)
